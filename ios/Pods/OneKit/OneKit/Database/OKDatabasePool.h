...
/** Pool of `<OKDatabase>` objects.
 ...
 @warning Before using `OKDatabasePool`, please consider using `<OKDatabaseQueue>` instead.

 If you really really really know what you're doing and `OKDatabasePool` is what
 you really really need (ie, you're using a read only database), OK you can use
 it.  But just be careful not to deadlock!

 For an example on deadlocking, search for:
 `ONLY_USE_THE_POOL_IF_YOU_ARE_DOING_READS_OTHERWISE_YOULL_DEADLOCK_USE_OKDATABASEQUEUE_INSTEAD`
 in the main.m file.
 */

@interface OKDatabasePool : NSObject
...
/** Create pool using path.
 ...
 @return The `OKDatabasePool` object. `nil` on error.
 */
...
/** Synchronously perform database operations in pool.
 ...
 @param block The code to be run on the `OKDatabasePool` pool.
 */
...
/** Synchronously perform database operations in pool using save point.
 ...
 @warning You can not nest these, since calling it will pull another database out of the pool and you'll get a deadlock. If you need to nest, use `<[OKDatabase startSavePointWithName:error:]>` instead.
*/
...
/** OKDatabasePool delegate category
 
 This is a category that defines the protocol for the OKDatabasePool delegate
 */

@interface NSObject (OKDatabasePoolDelegate)

/** Asks the delegate whether database should be added to the pool. 
 
 @param pool     The `OKDatabasePool` object.
 @param database The `OKDatabase` object.
 
 @return `YES` if it should add database to pool; `NO` if not.
 
 */

- (BOOL)databasePool:(OKDatabasePool*)pool shouldAddDatabaseToPool:(OKDatabase*)database;
...
- (void)databasePool:(OKDatabasePool*)pool didAddDatabase:(OKDatabase*)database;
