...
/** Represents the results of executing a query on an `<OKDatabase>`.
...
@interface OKResultSet : NSObject

@property (nonatomic, retain, nullable) OKDatabase *parentDB;
...
/** `OKStatement` used by result set. */

@property (atomic, retain, nullable) OKStatement *statement;

///------------------------------------
/// @name Creating and closing a result set
///------------------------------------

/** Create result set from `<OKStatement>`
 
 @param statement A `<OKStatement>` to be performed
 
 @param aDB A `<OKDatabase>` to be used
 
 @return A `OKResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(OKStatement *)statement usingParentDatabase:(OKDatabase*)aDB;
